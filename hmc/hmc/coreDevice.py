...
class coreDevices ():
    ...
    def restoreDevice(self,device):
        try:
            ...
            self.logger.error("can not restore device:%s"%(device))
            raise Exception
    ...
    def setDeviceAttribute(self,deviceID,attribute,value):
        ...
        try:
            ...
            self.logger.error("can not set attribute %s for device id %s value %s"%(attribute,deviceID,value))
            raise  
    ...
    def getDeviceAttributeValue(self,deviceID,attribute):
        try: 
            ...
            self.logger.error("can not get attribute %s for device id %s"%(attribute,deviceID))
            raise 
    ...
    def updateDevice(self,device):
        try:
            ...
            self.logger.error("can not update device %s"%(device['deviceID']['value']))  
            raise Exception
    ...
    def __buildDevice(self,device):
        ...
        try:
            ...
            try:
                ...
                self.logger.error("can not load default device")
            ...
            self.logger.emergency("can not load device")
            self.logger.error(sys.exc_info())
            tb = sys.exc_info()
            for msg in tb:
                self.logger.error("Traceback Info:%s"%(msg))  
